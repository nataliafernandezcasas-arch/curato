...
export default function CreadoresPage() {
  const { lang } = useLang();
  const t = translations[lang].creators;
      ...
      <main>
        ...
        <section className="relative min-h-[92vh] flex items-end pt-36 pb-20 overflow-hidden">
          <div className="absolute inset-0">
            <MouseParallaxImage src="/Background Image 4.jpeg" className="absolute inset-0" strength={10} />
        ...
        <section className="py-28 px-5 bg-charcoal-deep">
          <div className="max-w-[1200px] mx-auto">
            ...
            <motion.h2 {...fade(0.1)} className="font-serif text-4xl md:text-5xl font-light tracking-[0.28em] uppercase leading-[1.1] text-text-primary mb-20">
              {t.howTitle}
            </motion.h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-border">
              {t.steps.map((step, i) => (
                <motion.div key={step.num} {...fade(i * 0.1)} className="bg-charcoal-deep p-10 md:p-12">
                  <p className="font-serif text-[11px] tracking-[0.3em] text-champagne/40 mb-8">{step.num}</p>
                  <h3 className="font-serif text-2xl font-light tracking-wider uppercase text-text-primary mb-5">{step.title}</h3>
        ...
        <section className="py-28 px-5 bg-charcoal-mid">
          <div className="max-w-[1200px] mx-auto">
            ...
            <motion.h2 {...fade(0.1)} className="font-serif text-4xl md:text-5xl font-light tracking-[0.28em] uppercase leading-[1.1] text-text-primary mb-16">
              {t.whatTitle}
            </motion.h2>
            <div className="space-y-0 divide-y divide-border">
              {t.benefits.map((b, i) => (
                <motion.div key={b.num} {...fade(i * 0.08)} className="flex items-start gap-10 py-10">
                  <span className="font-serif text-[13px] tracking-[0.2em] text-champagne/40 w-10 shrink-0 pt-1">{b.num}</span>
                  <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-4">
                    <h3 className="font-serif text-2xl font-light tracking-wider uppercase text-text-primary">{b.title}</h3>
                    <p className="font-serif text-[15px] font-light text-text-muted leading-relaxed">{b.desc}</p>
                  </div>
                </motion.div>
              ))}
            </div>
