...
// ─────────────────────────────────────────────────────────────────────────────
// /invitacion — Client component for the "request invitation" landing.
//
// Same SSR-first pattern as SelectedClient: we use a local useState seeded
// with `initialLang` (passed from page.tsx based on ?lang= query param) so
// the very first paint (server + client hydration) is in the right language,
// no flicker. The global LanguageContext is synced via useEffect afterwards
// so the rest of the app picks up the language choice.
// ─────────────────────────────────────────────────────────────────────────────
export default function InvitacionClient({ initialLang }: { initialLang: Lang }) {
