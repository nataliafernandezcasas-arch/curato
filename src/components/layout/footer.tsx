        ...
        <div className="flex flex-col md:flex-row items-start md:items-end justify-between gap-10">
          <div>
            ...
            <p className="font-serif text-[13px] text-text-muted mt-2 tracking-wide font-light">
              Paris, France · Sur invitation
            </p>
          </div>
          <div className="flex gap-8 text-[12px] text-text-muted font-serif font-light tracking-widest">
            <Link href="/creadores" className="hover:text-champagne transition-colors">Créateurs</Link>
            <Link href="/comercios" className="hover:text-champagne transition-colors">Maisons</Link>
            <Link href="/auth/sign-in" className="hover:text-champagne transition-colors">Accéder</Link>
          </div>
        </div>
        <div className="border-t border-border mt-10 pt-8">
          <p className="font-serif text-[11px] text-text-muted/50 font-light italic leading-relaxed">
            © {new Date().getFullYear()} Curato. Tous droits réservés.
          </p>
        </div>
      </div>
    </footer>
